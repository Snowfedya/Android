...
class HomeFragment : BaseFragment() {
    ...
    private fun setupWelcomeMessage() {
        ...
        // Add click listener to welcome text or a new button to go to settings
        // For simplicity, let's make the welcome text clickable or add a menu item
        // But since we don't have a menu setup in this simple lab, let's just use the text
        binding.tvWelcome.setOnClickListener {
            androidx.navigation.fragment.findNavController().navigate(R.id.action_homeFragment_to_settingsFragment)
        }
        
        // Also add a toast hint
        Toast.makeText(requireContext(), "Click welcome text for Settings", Toast.LENGTH_LONG).show()
    }
    ...
    private fun loadRestaurantData() {
        ...
        lifecycleScope.launch {
            try {
                ...
                // Handle error (fallback to local data for demo purposes)
                Toast.makeText(requireContext(), "Network error: ${e.message}. Loading local data.", Toast.LENGTH_LONG).show()
                loadLocalData()
                binding.progressBar.visibility = View.GONE
                binding.recyclerView.visibility = View.VISIBLE
            }
